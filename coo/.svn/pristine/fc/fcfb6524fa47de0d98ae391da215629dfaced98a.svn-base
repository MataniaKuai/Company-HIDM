...
$('input[name="reworkCost"]').change(function() {
   var reworkCost = $(this).val();
   var scrapCost = $('input[name="scrapCost"]').val();
   var userBaseId = $('input[name="userBaseId"]').val();
   if(reworkCost.length > 0 && scrapCost.length > 0){
	   var param = {reworkCost: reworkCost, scrapCost: scrapCost, userBaseId: userBaseId};
	   $.post('/coo/opBase/costValue', param, function(result){
		   if (result){
			   $('input[name="waferCostRework"]').val(result.waferCostRework)
			   $('input[name="waferCostScrap"]').val(result.waferCostScrap)
		   } else {
			   alert('系统错误');
		   }
	   },'json');
...
$('input[name="scrapCost"]').change(function() {
	   var scrapCost = $(this).val();
	   var reworkCost = $('input[name="reworkCost"]').val();
	   var userBaseId = $('input[name="userBaseId"]').val();
	   if(reworkCost.length > 0 && scrapCost.length > 0){
		   var param = {reworkCost: reworkCost, scrapCost: scrapCost, userBaseId: userBaseId};
		   $.post('/coo/opBase/costValue', param, function(result){
