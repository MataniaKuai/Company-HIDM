function add(){
	...
	$('input[name="dspace"]').val("");
	$('input[name="wspace"]').val("");
...
function putFormValue() {
	...
	if (selects.length == 1) {
		...
		var dspaceV =selects[0].dspace;
		var wspaceV =selects[0].wspace;
		...
		$('input[name="dspace"]').val(dspaceV);
		$('input[name="wspace"]').val(wspaceV);
		...
		$('input[name="coment"]').val(comentV);
		if(updateUserIdV != null && updateUserIdV.length > 0){
				...
				if (result){
					for (var i=0; i < result.length; i++)
					{
						...
						'<select class="form-control" name="materialName" style="min-width:200px;">'+
							str+
						'</select>'+
						'<label class="input-group-addon">Usage By KMove:</label>'+
...
function save(){
	...
	var dspaceV = $('input[name="dspace"]').val();
	var wspaceV = $('input[name="wspace"]').val();
	...
	var comentV = $('input[name="coment"]').val();
	if(isNaN(lengthV) || isNaN(widthV) || isNaN(dspaceV) || isNaN(wspaceV)){
		alert("DS And WS And DT And WT 必须为数字！");
		return;
	}
    ...
    if(checked){
    	...
    	$.post('/coo/userBase/save',
    		...
    		width: widthV,dspace:dspaceV,wspace:wspaceV, housing: housingV, manMachine: manMachineV, shift: shiftV, uph: uphV,
    		...
    		addUserId:addUserIdV, updateUserId:updateUserIdV , coment:comentV
    			},
    			function(result){
    				if (result){
    					doSearch();
...
function userSubmit(){
	...
	//校验utility或者material价格是否为空
	var param2 = {utilityName: utilityNameArrV.toString(), materialName: materialNameArrV.toString()};
	$.ajax({
		url: "/coo/userBase/checkPrice",
		type: 'POST',
		cache: false,
		async:false,
		data: param2,
    ...
    if(checked){
      	...
      	$('#saveBtn').attr("disabled", true);
      	$('#submitBtn').attr("disabled", true);
    	$.post('/coo/userBase/submit',
    			{
    		eqTypeId: eqTypeIdV, module: moduleV, vendor: vendorV, model: modelV, length: lengthV,
    		width: widthV, wspace: wspaceV, dspace:dspaceV, housing: housingV, manMachine: manMachineV, shift: shiftV, uph: uphV,
    		uptimeRate: uptimeRateV, reworkRate: reworkRateV, scrapRate: scrapRateV, installationCost: installationCostV,
    		hookupCost: hookupCostV, installationHookupCost: installationHookupCostV, qualificationCost: qualificationCostV,
    		partsCost: partsCostV, utilityName: utilityNameArrV.toString(), utilityUsage:utilityUsageArrV.toString(),
    		materialName: materialNameArrV.toString(), materialUsage:materialUsageArrV.toString(),id:idV, addUserId:addUserIdV, updateUserId:updateUserIdV ,coment:comentV
    			},
    			function(result){
    				if (result){
    					doSearch();
...
function changLength() {
	 ...
	 var dspace = $('input[name="dspace"]').val();
	 var wspace = $('input[name="wspace"]').val();
	 if(isNaN(length)){
		 $(this).blur();
		 alert("Length 不能包含非数字字符！");
	 }
	 if(length != "" && length != null && width != "" || width != null){
		 $('input[name="housing"]').val(((Number(length) + Number(dspace)) * ( Number(width ) + Number( wspace ) ))/4);   
	 }
}
