...
function putFormValue2(row) {
	var courseIdV = row.courseId;
	var titleV = row.title;
	var phaseV = row.phase;
	var weekV = row.week;
	var durationV = row.duration;
	var addressV = row.address;
	var lecturerNameV = row.lecturerName;
	var courseStartDateV = row.courseStartDate;
	if (courseStartDateV && courseStartDateV.length >= 19) {
		courseStartDateV = courseStartDateV.substring(0, 16);
	}
	var courseEndDateV = row.courseEndDate;
	if (courseEndDateV && courseEndDateV.length >= 19) {
		courseEndDateV = courseEndDateV.substring(0, 16);
	}
	var fileNameV = row.fileName;
	var filePathV = row.filePath;
	var scoreV = row.score;
	var idV = row.id;
	if(null != scoreV && "" != scoreV){
		var scoreVInt = parseInt(scoreV);
		$('input:radio[name=scoreRadiosinline]')[scoreVInt - 1].checked = true;
