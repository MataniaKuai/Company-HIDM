   ...
   $.fn.pickList = function (options) {
      ...
      this.controll = function () {
         ...
         pickThis.find(".pRemove").on('click', function () {
            ...
            //从右往左添加数据时,对左边数据进行排序
  		    var objData = [];
  		    var objDataMap = new Map();
  		    pickThis.find(".pickData option").each(function () {
  		    	objData.push(
  		    		$(this).data('id')
  		  	    );
  		  	    objDataMap.set($(this).data('id'),this.text);
  		    });
  		    objData.sort();
  		  
  		    var option = '';
  		    for(var i = 0,len=objData.length; i < len; i++) {
  		        option += '<option data-id=' + objData[i] + '>' + objDataMap.get(objData[i]) + '</option>';
  		    }
  		    pickThis.find('.pickData').html(option);
         });
